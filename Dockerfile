...
RUN echo "deb http://packages.inverse.ca/SOGo/nightly/4/ubuntu/ bionic bionic" > /etc/apt/sources.list.d/SOGo.list && \
    ...
    apt-get update && \
    apt-get install -y --no-install-recommends gettext-base iproute2 net-tools apache2 sogo sogo-activesync libwbxml sope4.9-gdl1-mysql memcached inetutils-ping inetutils-traceroute && \
